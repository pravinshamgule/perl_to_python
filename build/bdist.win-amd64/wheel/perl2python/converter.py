...
class PerlToPythonConverter:
        ...
        for line in perl_body.strip().split('\n'):
            ...
            if match:
                ...
                for var_name in re.findall(r'\b(\w+)\b', text):
                    if var_name in ['and', 'or', 'not', 'is', 'in', 'for', 'while', 'if', 'else', 'elif', 'try', 'except', 'finally', 'with', 'as', 'def', 'class', 'return', 'yield', 'from', 'import', 'True', 'False', 'None']:
                        continue  # Skip Python keywords
    ...
    def _convert_perl_condition(self, condition: str) -> str:
        ...
        # Convert Perl defined check
        condition = re.sub(r'defined\s+\$?(\w+)', r'\1 is not None', condition)
